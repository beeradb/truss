...
// "<kind>\t<path>" -- kind being repo.Kind's String(): credentials, tofu,
// ansible or render. This is the subcommand render_unit.go's renderUnitsFor
// names as the thing that makes "CI derives the units with this same
// function" true: it derives the render half of a commit's units by feeding
// the exact same inputs -- ChangedFiles and TreeRenderUnits -- into the exact
// same repo.TouchedUnits the apply pass calls, rather than a consumer's CI
// job maintaining its own approximation of the rule. See docs/work-items.md
...
func cmdUnits(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	const usageMsg = "usage: truss units <sha> [--dir <path>] [--kind credentials|tofu|ansible|render]"
	...
	if kindSet {
		...
		if !ok {
			fmt.Fprintf(stderr, "units: unknown --kind %q (want credentials, tofu, ansible or render)\n", kindFlag)
			return 2
		}
		wantKind = k
	}

	// The same driver and the same two calls render_unit.go's renderUnitsFor
	// is fed from in the apply pass (apply_cmd.go): ChangedFiles for what the
	// commit changed, TreeRenderUnits for what render units exist in its own
	// tree. No token: this reads a checkout already on disk and nothing on
	// the network.
	...
	treeRenderUnits, err := g.TreeRenderUnits(ctx, sha)
	if err != nil {
		fmt.Fprintf(stderr, "units: %v\n", err)
		return 1
	}

	for _, u := range repo.TouchedUnits(changedFiles, treeRenderUnits) {
...
func kindFromString(s string) (repo.Kind, bool) {
	...
	case "ansible":
		return repo.KindAnsible, true
	case "render":
		return repo.KindRender, true
	}
	return 0, false
}
